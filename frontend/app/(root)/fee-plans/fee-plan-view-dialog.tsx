...
export function FeePlanViewDialog({
  feePlan,
  open,
  onOpenChange,
}: {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-2.5">
            <DialogTitle className="truncate">{feePlan?.name ?? "Fee plan"}</DialogTitle>
            {feePlan && <ProductStatusBadge status={feePlan.status} />}
          </div>
          <DialogDescription>Fee plan details.</DialogDescription>
        </DialogHeader>

        {feePlan && (
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              <Field label="Program" value={feePlan.program.name} />
              <Field
                label="Duration type"
            ...
            <Field
              ...
              value={
                <p className="rounded-lg bg-muted/40 p-3 text-sm font-normal text-foreground/90">
                  {feePlan.description || "No description provided."}
                </p>
              }
            />

            <div className="grid grid-cols-2 gap-x-6 gap-y-4 border-t pt-4">
              <Field label="Created" value={<DateText value={feePlan.createdAt} withTime />} />
              <Field label="Last updated" value={<DateText value={feePlan.updatedAt} withTime />} />
            </div>
          </div>
        )}
      </DialogContent>
