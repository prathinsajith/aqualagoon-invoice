...
import { TrainingTypeFormDialog } from "./training-type-form-dialog";
import { TrainingTypeViewDialog } from "./training-type-view-dialog";
import { useTrainingTypes, useTrainingTypeMutations } from "@/hooks/queries/use-training";
...
export function TrainingTypesContent() {
  ...
  const { data, isLoading, isError, error } = useTrainingTypes(params);
  const { remove } = useTrainingTypeMutations();

  const [formOpen, setFormOpen] = useState(false);
  const [editing, setEditing] = useState<TrainingType | null>(null);
  const [viewing, setViewing] = useState<TrainingType | null>(null);
  const [pendingDelete, setPendingDelete] = useState<TrainingType | null>(null);

  const resetToFirstPage = () => setPagination((p) => ({ ...p, pageIndex: 0 }));

  const columns = useMemo(
    () =>
      getTrainingTypeColumns({
        can,
        onView: setViewing,
        onEdit: (c) => {
          setEditing(c);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Training Types" description="Group training programs into types.">
        <Can permission="training_type.create">
          <Button
            onClick={() => {
              setEditing(null);
              setFormOpen(true);
          ...
          >
            <IconPlus className="size-4" /> New training type
          </Button>
        </Can>
      </PageHeader>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        <RefreshButton queryKey={["training-types"]} className="sm:ml-auto" />
      </div>

      {isError ? (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 p-6 text-center text-sm text-destructive">
          {getApiErrorMessage(error, "Failed to load training types")}
        </div>
      ) : (
        <DataTableGeneric
      ...
      <TrainingTypeFormDialog open={formOpen} onOpenChange={setFormOpen} trainingType={editing} />
      <TrainingTypeViewDialog
        trainingType={viewing}
        open={!!viewing}
      ...
      <ConfirmDialog
        ...
        description="The training type will be archived. Types with existing programs can't be deleted."
        confirmLabel="Delete"
        destructive
...
export default function TrainingTypesPage() {
  return (
    <PermissionPage permission="training_type.view">
